...
### 2026-06-23 (全链路体验优化：星空图谱交互与原文对照解析)
- **【星空图谱全息升级 (ForceGraph)】**：
  - 将无机质的颜色球替换为生动的 **Emoji 图标**（📄合同、🏢企业、🧑经办人），大幅提升视觉辨识度。
  - 引入了**动态体积算法**：合同节点的大小根据其金额成正比放大（并增加了最大倍数上限防止霸屏），核心业务一目了然。
  - 强化了依赖追踪：针对重要的“销售-采购”依赖关系链路，采用加粗的**红色高亮连线**，直击风险传导路径。
  - 重构了交互逻辑：点击节点不再突兀跳转，而是在右侧平滑滑出 **Drawer 抽屉面板** 展示节点详细属性，需要深度编辑时再通过底部按钮跳转。
- **【合同解析链路增强与双屏对照 (Document Parser)】**：
  - **持久化原件归档**：重写了上传接口，所有解析的合同原件（PDF/图片）将被永久归档至 `uploads/contracts/` 目录并生成访问 URL 供随时查阅。
  - **多模态与容错增强**：升级了 LLM 解析 Prompt，除了新增时间节点、主办部门等字段外，要求模型输出每个字段的**置信度 (confidence)**。加入了三级 JSON 容错解析机制（Markdown脱壳 → 正则提取 → 逗号修复），杜绝解析崩溃。
  - **左右分栏的沉浸式导入**：彻底重构 `ImportPanel.tsx`，采用 **50/50 左右分栏布局**。左侧渲染合同原文，右侧展示提取结果。针对 LLM 标注的“低置信度”字段用红色边框和 Warning Tag 高亮，引导用户重点核对。
  - **台账随时溯源**：抽离出通用的 `ContractFileViewer`（支持 PDF 与图片），在 `LedgerPanel` 中新增了"查看原文"功能，点击后左侧滑出原文画卷，实现“左看原文，右看台账”。

### 2026-06-22 (工程化涅槃：企业级架构重构与 SQLite 彻底去 COM 化)
- **【全景目录规范治理 (Monorepo-like)】**：
  - 彻底终结了根目录下文件散落的局面，建立了标准的工程化目录结构。
  - 新建 `docs/` 收纳 PRD 与开发日志，`data/` 存放测试数据与参考资料，`deploy/` 存放构建产物。
- **【后端架构五层拆分 (FastAPI)】**：
  - 将原有的巨无霸 `main.py` 连根拔起，重构为现代化的五层架构：
    - `api/`：API 路由层（Auth, Ledger, Chat, System, Upload等分类存放）。
    - `core/`：核心配置与底层组件（Config, LLMClient, DB）。
    - `services/`：业务逻辑层（LedgerService）。
    - `agents/`：AI 智能体层（ChatAgent, ParserAgent），内部划分为 prompts, tools 等。
    - `models/`：数据模型（保留位置用于后续的 Pydantic Schema）。
- **【前端目录深度梳理 (React/Vite)】**：
  - 终结了所有代码塞在 `components/` 的历史。
  - 建立标准前端基建：`api/` (接口)、`pages/` (路由视图)、`components/` (复用组件)、`hooks/` (自定义钩子)、`types/` (类型定义)。
- **【底层数据持久化革命 (SQLite 替换 Excel)】**：
  - **彻底移除 xlwings** 与对 Windows COM 组件的深度依赖，解决了“服务器上跑不起来”的致命隐患。
  - 引入了 SQLite (`contracts.db` 与 `users.db`) 作为核心数据载体。所有台账的增删改查、排序、以及审计日志 (Audit Log) 均在数据库内极速完成。
  - 保留了轻量级的 `openpyxl`，仅在系统首次启动且数据库为空时，负责将原有的 `mock_ledger.xlsx` 无损清洗并导入至 SQLite，实现平滑过渡。
...
### PDF 解析与高亮精确定位深度优化
- **底层通信修复**：修复了前端 `action` 与后端 `tool_name` 参数不匹配导致“确认录入台账”假成功的问题，实现了合同记录真正写入数据库。
- **渲染引擎升级**：移除了前端粗暴的全局黄色段落高亮和调试面板，引入了 `activeBbox` 单点跟踪高亮和丝滑自动滚动交互。
- **取消失焦隐藏**：去除了焦点丢失导致高亮框闪退的逻辑，确保用户核对原文档时体验更加稳固。
- **像素级精度定位**：在后端坐标提取层深度集成 `pdfplumber`。优先通过全文词组级搜索，精准反套绝对坐标，实现字级（Pixel-perfect）严丝合缝的高亮框；遇扫描件等特殊情况则自动降级使用 MinerU/PaddleOCR 的段落级大模型视觉坐标。
